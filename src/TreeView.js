...
function App() {
  ...
  const handleApplyWhereCondition = () => {
    global.whereClause = `WHERE ${selectedWhereColumn} ${selectedWhereOperator} ${whereValue}`;
    // Use whereClause in your query building logic
    setOpenWhereDialog(false);
  };
  useEffect(() => {
  ...
  const handleQueryGenerate = () => {
      ...
      for (let i = 0; i < selectedTables.length - 1; i++) {
        ...
        if (commonColumns.length > 0) {
          ...
          joinCondition = commonColumns
            ...
            .join(" AND ");
          joinClauses.push(
            `INNER JOIN ${selectedTables[i + 1]} ON ${joinCondition}`
          );
        }
      }
    ...
    if (selectedColumns.length > 0) {
      const whereOptions = [">", "<", ">=", "<=", "="];
      const whereCondition = prompt(
        `Select a where condition:\n\n${whereOptions
          .map((option) => ` ${option}`)
          .join("\n")}\n\n(leave empty for no condition)`
      );
      if (whereCondition) {
        const selectedColumn = prompt(
          "Enter the column name for the WHERE condition:"
        );
        const selectedValue = prompt(
          "Enter the value for the WHERE condition:"
        );
        whereClause = `WHERE ${selectedColumn} ${whereCondition} '${selectedValue}'`;
      }
    }
                ...
                {selectedDatabase && (
                  <div
                    style={{
                      ...
                      alignItems: "center",
                      marginTop: "1rem",
                      justifyContent: "space-between",maxHeight:'25rem',overflowY:'scroll',width:'80%'
                    }}
                  >
                    <h6
                      style={{
    ...
    <div className="col-md-8" >
            <form onSubmit={handleSubmit} style={formStyle}>
              ...
              <textarea
                ...
                name="query"
                value={finalQuery}
                placeholder="Enter Ypur SQL Query "
                ...
                {activeTab === "result" && results.length > 0  ? (
                  ...
                  <div style={{ marginTop: "1rem" }}>
                   <div
                    ...
                    >
                          <button
                            onClick={() => setFinalQuery(prevQuery => prevQuery + "Select")}
                            style={{ width: "10rem" }}
                          >
                            Select
                          </button>
                          <button
                            onClick={() => setFinalQuery(prevQuery => prevQuery + "*")}
                            style={{ width: "10rem" }}
                          >
                            *
                          </button>
                          <button
                            onClick={() => setFinalQuery(prevQuery => prevQuery + "From")}
                            style={{ width: "10rem" }}
                          >
                            From
                          </button>
                          <button
                            onClick={() => setFinalQuery(prevQuery => prevQuery + "Join")}
                            style={{ width: "10rem" }}
                          >
                            Join
                          </button>
                          <button
                            onClick={() => setFinalQuery(prevQuery => prevQuery + "Inset")}
                            style={{ width: "10rem" }}
                          >
                          Insert
                          </button>
